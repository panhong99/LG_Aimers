...
@torch.no_grad()
def measure_proxy_perf_ppl(
    model, tokenizer, prompts: List[str], max_len: int, device: torch.device
) -> float:
    model.eval()
    model.to(device)
    total_loss = 0.0
...
def try_measure_token_time_vllm(
    model_path: str, tokenizer_path: str, prompts: List[str], max_new_tokens: int
) -> float:
...
@torch.no_grad()
def measure_token_time_hf(
    model, tokenizer, prompts: List[str], max_new_tokens: int, device: torch.device
) -> float:
    model.eval()
    model.to(device)
...
def main():
    ...
    if (not args.no_vllm) and device.type == "cuda":
        try:
            token_time_base = try_measure_token_time_vllm(
                args.base, args.base, prompts, args.max_new_tokens
            )
            token_time_model = try_measure_token_time_vllm(
                args.model, args.model, prompts, args.max_new_tokens
            )
        except Exception as e:
            print(f"[WARN] vLLM 측정 실패 → HF fallback 사용: {e}")

    if token_time_base is None or token_time_model is None:
        token_time_base = measure_token_time_hf(
            base_model, tok, prompts, args.max_new_tokens, device
        )
        token_time_model = measure_token_time_hf(
            model, tok, prompts, args.max_new_tokens, device
        )
